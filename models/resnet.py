...
class ResNet(nn.Module):
    def __init__(self, block, layers, drop_perc=0.333, jigsaw_classes=1000, classes=100):
        self.inplanes = 64
        ...
        self.conv_attention = nn.Conv2d(2, 1, 3, padding=1)
        ...
        # self.pecent = 1/2 # batch percentage with RSC
...
#                 self.pecent = 3.0 / 10 + (epoch / interval) * 2.0 / 10
            ...
            # ---------------------------- spatial -----------------------
            spatial_drop_num = math.ceil(HW * self.drop_perc)
        ...
        attention_mean = torch.mean(x, dim=1, keepdim=True)
        attention_max = torch.max(x, dim=1, keepdim=True)[0]

        attention = torch.cat((attention_mean, attention_max), 1)
        
        attention = self.conv_attention(attention)
        self.attention = self.sigm(attention)
